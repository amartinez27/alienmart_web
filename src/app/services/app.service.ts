...
export class AppService {
    ...
    constructor(public http: HttpClient, public snackBar: MatSnackBar, public catService: CategoriesService,
        public prodService: ProductService, public auth: AngularFireAuth, public wishlistService: WishlistService) { }
    ...
    public getProducts(type): Observable<Product[]> {
        ...
        return this.prodService.getProducts(1, 6);
    }
    ...
    public addToCompare(product: Product) {
        ...
        this.snackBar.open(message, '×', { panelClass: [status], verticalPosition: 'bottom', duration: 3000 });
    }

    public async addToWishList(product) {
        let message, status;
        if (this.Data.wishList.filter(item => item.id == product.id)[0]) {
            ...
            (await this.auth.currentUser).getIdToken().then(token => {
                if (product.featured_type_id) {
                    this.wishlistService.createAdLike(product.id, token).subscribe(like => {
                        ...
                        status = 'warning';
                        this.snackBar.open(error.error?.error, '×', { panelClass: [status], verticalPosition: 'bottom', duration: 3000 })
                    })
                } else {
                    this.wishlistService.createLike(product.id, token).subscribe(like => {
                        this.Data.wishList.push(product);
                        product.liked = true;
                        product.likes++;
                        message = product.name + ' ha sido añadido a tus favoritos.';
    ...
    public addToCart(product: Product) {
        ...
        this.Data.totalcart_count = null;
        ...
        this.Data.cartList.forEach(product => {
            this.Data.totalPrice = this.Data.totalPrice + (product.cart_count * product.price);
            this.Data.totalcart_count = this.Data.totalcart_count + product.cart_count;
        });

        message = product.name + ' ha sido añadido a tu carrito de compras.';
        status = 'success';
        this.snackBar.open(message, '×', { panelClass: [status], verticalPosition: 'bottom', duration: 3000 });
    }

    public resetProductcart_count(product: Product) {
        product.cart_count = 0;
        let compareProduct = this.Data.compareList.filter(item => item.id == product.id)[0];
        if (compareProduct) {
            compareProduct.cart_count = 0;
        };
        let wishProduct = this.Data.wishList.filter(item => item.id == product.id)[0];
        if (wishProduct) {
            wishProduct.cart_count = 0;
        };
    }
    ...
    public getDeliveryMethods() {
        return [
            { value: 'free', name: 'Envío gratis', desc: '$0.00 / Entrega de 1-3 días hábiles' }
            //{ value: 'standard', name: 'Entrega estándar', desc: '$149.00 / Entrega de 5-7 días hábiles' }
